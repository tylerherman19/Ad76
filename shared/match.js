 ...
 * GIS ward layer.
 *
 * PURE MODULE — takes the ward index and a warning sink, so the Node backend
 * logs to stdout/`/api/logs` and the browser build logs to its diagnostics
 * panel, from identical matching logic.
 *
 ...
 * `unmatched` with the reason — never dropped, and never snapped to a "close
 * enough" ward.
 */

import { parsePrecinctName, displayWardLabel } from './precinctName.js';

export function matchUnitsToWards(units, wardIndex, warn = () => {}) {
  const { byKey, wards } = wardIndex;
  ...
  for (const unit of units) {
    ...
    if (!parsed.ok) {
      ...
      warn('match.unparseable_precinct', problem);
      continue;
    }
    ...
    if (!hits.length) {
      ...
      unmatched.push(problem);
      warn('match.no_ward_found', problem);
      continue;
    }
    ...
    if (misses.length) {
      ...
      unmatched.push(problem);
      warn('match.partial_ward_coverage', problem);
    }
  ...
  // night this is normally empty once the county posts the race; if it is not,
  // the district's ward list and the county's precinct list disagree and that
  // needs to be visible.
  ...
  if (wardsWithoutResults.length) {
    warn('match.wards_with_no_reporting_unit', {
