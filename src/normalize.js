...
import { displayWardLabel, shortWardLabel } from './precinctName.js';
...
function leaderOf(rows, candidates) {
  ...
  return {
    ...
    margin,
    votes: first.votes,
    percent: pct(first.votes, total),
  };
}

export function buildPayload(raw) {
  const candidates = assignCandidateColors(raw.candidates ?? []);
  const colorOf = new Map(candidates.map((c) => [c.name, c.color]));
  const { matched, unmatched, wardsWithoutResults, summary } = matchUnitsToWards(raw.units ?? []);
  const { byKey } = loadWards();
  ...
  const units = matched.map((u) => {
    ...
    return {
      ...
      candidates: reported
        ? rows
        : rows.map((r) => ({ name: r.name, votes: null, percent: null })),
      leader: leader
        ? {
            name: leader.name,
            tied: leader.tied,
            ...
            strength: marginStrength(leader.margin),
          }
        : null,
    };
  });
  ...
  // true for any ward the county has not yet listed). These are explicit
  // "not reporting" rows — structure without invented numbers.
  for (const w of wardsWithoutResults) {
    const ward = byKey.get(w.wardKey);
    const label = `${ward.municipality.replace(/^City of /, '')} Ward ${ward.wardNumber}`;
    units.push({
      id: w.wardKey,
      label: ward.municipality.startsWith('City') ? label : `${ward.municipality} Ward ${ward.wardNumber}`,
      shortLabel: label,
      precinctName: null,
      precinctNumber: null,
      wardKeys: [w.wardKey],
  ...
  // together; the tooltip names every ward in the unit so this is not hidden.
  const wardFill = {};
    ...
    .map((g) => {
      ...
      for (const c of candidates) {
        ...
        tally[c.name] = reportedMembers.length
          ? reportedMembers.reduce(
              (s, m) => s + (m.candidates.find((r) => r.name === c.name)?.votes ?? 0),
              0,
            )
          : null;
      }
      const { total, rows } = summarize(tally, candidates);
      const anyReported = members.some((m) => m.reported);
      return {
        ...g,
        unitIds: g.unitIds.sort((a, b) =>
          (unitById.get(a).label ?? '').localeCompare(unitById.get(b).label ?? '', undefined, { numeric: true }),
        ),
        reportedUnits: members.filter((m) => m.reported).length,
    ...
    summary: {
      ...
      leader: grandLeader
        ? {
            ...
            color: grandLeader.tied ? null : assignCandidateColors(raw.candidates).find((c) => c.name === grandLeader.name)?.color ?? null,
          }
        : null,
    },
    units,
